...
import { Search, Save, X, CheckCircle, User, Users } from 'lucide-react';
import { useDataContext } from '../../backend/DataLoader';

const StudentScoreEntry = ({ disziplinId, disziplinName = "Unbekannte Disziplin", teamId }) => {
  const { students, loading, error } = useDataContext(); // Schüler aus dem Context
  ...
  // Lade Schüler des ausgewählten Teams
  useEffect(() => {
    if (!loading && students && teamId) {
      const filtered = students.filter(student => student.TEAMID === teamId);
      setTeamStudents(filtered);
    }
  }, [students, teamId, loading]);

  const filteredStudents = teamStudents.filter(student =>
  ...
  const handleScoreSubmit = async (e) => {
    ...
    try {
      const ergebnisData = {
        SCHUELERID: selectedStudent.SCHUELERID,
        TEAMID: teamId,
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/schuelerErgebnisse`, {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        setNotification({
          type: 'success',
          message: `Punkte für ${selectedStudent.VORNAME} ${selectedStudent.NACHNAME} erfolgreich gespeichert.`
        });
    ...
    return (
      <div className="rounded-lg border bg-white shadow-sm p-6">
        <div className="flex justify-center items-center py-8">
          ...
          <span className="ml-3 text-slate-500">Schüler werden geladen...</span>
        </div>
      </div>
    );
  }
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-red-700">
                Fehler beim Laden der Schüler: {error}
              </p>
            </div>
          </div>
        </div>
  ...
  if (teamStudents.length === 0) {
    return (
      <div className="rounded-lg border bg-white dark:bg-slate-800 shadow-sm p-6">
        <div className="text-center p-6 bg-slate-50 dark:bg-slate-700/30 rounded-lg">
          ...
          <p className="text-slate-600 dark:text-slate-300">Keine Schüler in diesem Team verfügbar.</p>
        </div>
      </div>
    );
  }
        ...
        {!selectedStudent ? (
          <div>
            <div className="relative mb-4">
              ...
              <input 
                type="text" 
                placeholder="Schüler suchen (Name oder ID)..." 
                value={searchTerm}
            ...
            <div className={`max-h-80 overflow-y-auto rounded-md border border-slate-200 dark:border-slate-600 ${teamStudents.length === 0 ? 'hidden' : ''}`}>
              {searchTerm.length > 0 && filteredStudents.length === 0 ? (
                  ...
                  {filteredStudents.map(student => (
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="font-medium text-slate-800 dark:text-white">{student.VORNAME} {student.NACHNAME}</p>
                          <div className="flex items-center mt-1">
                            <span className="text-xs text-slate-500 dark:text-slate-400">ID: {student.SCHUELERID}</span>
                            {student.KLASSE && 
            ...
            <div className="mb-6 p-4 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="font-medium text-slate-800 dark:text-white">{selectedStudent.VORNAME} {selectedStudent.NACHNAME}</p>
                    <div className="flex items-center mt-1">
                      <span className="text-xs text-slate-500 dark:text-slate-400">ID: {selectedStudent.SCHUELERID}</span>
                      {selectedStudent.KLASSE && 
                        <span className="text-xs text-slate-500 dark:text-slate-400 ml-2">
                          Klasse: {selectedStudent.KLASSE}
                        </span>
                      }
                    </div>
                  </div>
